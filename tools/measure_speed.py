...
V Max is a hard cap on the whole step - velocity PLUS strafe - so what has to
be measured is the distance a particle actually covers, not |vel|. The two are
not interchangeable: strafe is added straight to position, and for some presets
it is most of the movement. Measured from two consecutive position snapshots.

Its slider has to reach below the slowest preset and sit above the fastest, or
the control appears to do nothing over most of its travel. Both ends are
properties of the running sim, not of anything derivable on paper, so this steps
each preset and reads the entity buffer directly - no Camera and no CLIP.
...
def measure(preset_names, steps=400, seed=12345, limit=0):
    """-> [(stem, p50, p99, peak)] of |delta pos| in canvas units per step."""
    import moderngl
    ...
    print(f"{'preset':30s} {'p50':>10s} {'p99':>10s} {'peak':>10s} {'dropped':>8s}")
    ...
    for path in paths:
        ...
        sim.update(ctx)
        ctx.finish()
        mag = np.linalg.norm((positions() - before).astype(np.float64), axis=1)

        # A wrap, a bounce or a hazard respawn teleports a particle, and that
        # jump is not a step. Nothing legitimate crosses a quarter of the world
        # in one frame, so the cut is far above the signal.
        n = mag.size
        mag = mag[np.isfinite(mag) & (mag < 0.5)]
        if not mag.size:
            continue
        dropped = 100.0 * (n - mag.size) / n
...
def report(rows):
    ...
    units are |dpos| * sqrt(world_size)."""
    if not rows:
        return
    from state.preferences_state import PreferencesState
    ...
    upper = np.array([r[2] for r in rows]) * scale
    peak = np.array([r[3] for r in rows]) * scale
    pdef = PARAM_BY_NAME["V_MAX"]

    # p99, not max. A hazard respawn or a boundary reset teleports a particle
    # any distance it likes, so the largest |dpos| in a preset is a boundary
    # event rather than a step, and no magnitude cut separates the two.
    slowest, fastest = float(typical.min()), float(upper.max())
    print("\n--- step distance in V Max units (|dpos| * sqrt(world_size)) ---")
    print(f"n                     {len(rows)}")
    print(f"slowest preset p50    {slowest:.6f}   ({min(rows, key=lambda r: r[1])[0]})")
    print(f"median preset p50     {float(np.median(typical)):.6f}")
    print(f"fastest preset p99    {fastest:.6f}   ({max(rows, key=lambda r: r[2])[0]})")
    print(f"largest single |dpos| {float(peak.max()):.6f}   "
          f"(includes respawns and boundary jumps - not a step)")
    ...
    print(f"  headroom above the fastest preset: {pdef.default_max / fastest:.1f}x")
