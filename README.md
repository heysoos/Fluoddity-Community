...
**Seed** is the number the audio weights are drawn from, and it is the rig's: type one in and press Enter to get exactly the weights it gave before, or **Reroll** for a new one. Each channel's weights depend only on the seed and the channel's own number, so adding a fifth channel never moves the first four. **Audio Scale** scales every audio weight at once, and at zero the brain is deaf again. The count, the seed, the channels and their rows are all saved with the rig, so a named rig reproduces its inputs on any brain.
