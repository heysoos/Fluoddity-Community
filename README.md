...
**Auto (Prompt)** — type a prompt, press **Set**, then **Start**. CMA-ES climbs the grid toward whatever the encoder scores as the closest match. Or set `Goal` to **Image** and **Browse** to a picture: the search steers toward that instead, scored picture-to-picture. `Distractors` scores it against blank and noise canvases so an empty grid cannot win, and is best left on. Drag the square on the preview to choose which part of the picture counts, and tighten it with `Crop Zoom`. `Grayscale` scores particle density alone, for either kind of goal, so colour can neither win nor lose a round. The small picture under the metrics is tile 0 exactly as the encoder saw it, which is the thing to look at when a setting seems not to bite. `Grid` is tiles per side (2–8), `Steps per Gen` is how long each generation runs before it is scored. Tick `Search Physics Too` to let it move the physics sliders as well as the brain. `Encoder` picks which vision model does the scoring — CLIP B/32 is the default and by far the fastest; the larger ones see finer detail and cost proportionally more per generation.
...
The top line names the brain and its input count. `-` and `+` change the count, which belongs to the RIG rather than to the preset: load any preset, switch any brain, and it is given that many inputs again, its rule carried across with the audio weights drawn from the seed. Going back to zero returns exactly the brain you had. **Feed** is the master switch. With every channel at zero, or Feed off, or capture stopped, the brain computes precisely what it did before it had ears — so any preset in the library can take audio inputs and lose nothing. A preset with no rule of its own (every cohort running a generated brain) keeps that promise too.

**Seed** is the number the audio weights are drawn from, and it is the rig's: type one in and press Enter to get exactly the weights it gave before, or **Reroll** for a new one. Each weight depends only on the seed, the channel's number and the unit it feeds, so adding a fifth channel never moves the first four, and a preset one neuron larger hears the same through every neuron the two share. **Audio Scale** multiplies every audio weight at once and touches nothing else: at zero the brain is exactly its deaf self. The count, the seed, the channels and their rows are all saved with the rig, so a named rig reproduces its inputs on any brain.

Below that, one row per channel, shaped like the rows in the Modulate tab: the six signal dots bind bands to it, and the drawer holds each band's shaper and cohort mask, plus a **Channel** tab with the channel's name and **Range** — what a full-scale sum reads as, in the brain's own input units. A channel is a signed sum: add rows push it up, subtract rows pull it below zero, and it is zero whenever its rows are. The readout under the top line shows what the brain is fed, or says why the channels are silent. The count and Reroll are unavailable while the tournament grid is running or a hover preview is borrowing the brain.

A **phase** shaper, here and in the Modulate tab, has an **Abs** box. Ticked, the wave runs 0 to 1, which is what every rig before the box was tuned on; unticked it swings -1 to 1 around the base, so a parameter breathes both ways rather than only up.
